...
const htmlEl = (elName, className, attributes = "") =>
  `<${elName} class="${className}" ${attributes}>${className}</${elName}>`;
const markUp = (elArr) => elArr.reduce((acc, el) => (acc += el), "");
// App elements
const header = htmlEl("header", "header", 'style="border: 1px solid black"');
const main = htmlEl("main", "main", 'style="border: 1px solid black"');
const aside = htmlEl("aside", "aside", 'style="border: 1px solid black"');
const footer = htmlEl("footer", "footer", 'style="border: 1px solid black"');
// Build App content
